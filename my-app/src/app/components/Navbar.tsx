...
import { HiOutlineChat } from "react-icons/hi"; // Updated icon
...
const Navbar = () => {
  ...
  const [balance, setBalance] = useState<string>('0 LK'); // Set initial balance to '0 KV'
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        // Fetch the balance here (for example purposes, set to 15 KV)
        setBalance('15 LK'); // Replace this with your logic to fetch balance
      } catch (error) {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 flex justify-between items-center p-6 bg-transparent z-50">
      <div 
        ref={logoRef} 
        className="flex items-center font-bold text-3xl text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-red-500"
      >
        <HiOutlineChat className="mr-2 text-yellow-300 text-2xl animate-bounce" />
        <a href="/">LexiKorea</a>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button 
          ...
          className="text-white bg-gradient-to-r from-teal-400 to-blue-600 hover:from-teal-300 hover:to-blue-500 transition duration-300 px-4 py-2 rounded-lg transform hover:scale-105"
        >
